...
import { requiredImports_jokuh_web } from "./requiredImports";

const loadSceneFromSceneData_jokuh_web = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_jokuh_web.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_jokuh_web.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_jokuh_web.jsFunctions) {
...
export { Poly, loadSceneFromSceneData_jokuh_web };
